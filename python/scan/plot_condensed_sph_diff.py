...
def is_logarithmic(edges, maxdev=1E-3):
    """
...
def get_bin_centers(edges):
    """
    Get the bin centers for a given set of bin edges. This works even if bins
    don't have equal width.
    """
...
data[:,6] = data[:,6].astype(np.float32)
data[:,7] = data[:,7].astype(np.float32)
data[:,8] = data[:,8].astype(np.float32)
...
for arr in sep_arrays:
    arr = arr.T
    sort_column = 7
    data_sorted = arr[arr[:,sort_column].argsort()]
    uni, c = np.unique(arr[:,sort_column], return_counts=True)
    ...
    sep_arrays_2 = []
    for x in xrange(n):
        sep_arrays_2.append(col_array[:,x])

    sub_array = []
    for arr_2 in sep_arrays_2:
        arr_2 = arr_2.T
        sort_column = 6
        data_sorted = arr_2[arr_2[:,sort_column].argsort()]
        uni, c = np.unique(arr_2[:,sort_column], return_counts=True)
        ...
        sep_arrays_3 = []
        for x in xrange(n):
            sep_arrays_3.append(col_array[:,x])
        sep_arrays_3 = np.array(map(np.hstack, sep_arrays_3))

        sub_array.append(sep_arrays_3)
...
data_comp[:,6] = data_comp[:,6].astype(np.float32)
data_comp[:,7] = data_comp[:,7].astype(np.float32)
data_comp[:,8] = data_comp[:,8].astype(np.float32)

argmin_comp = np.argmin(data_comp[:,9])
min_entry_comp = data_comp[argmin_comp,:]
min_llh_comp = min_entry_comp[9]
# min_llh_comp = 1.21337335e+03

print 'data', data_comp.shape

sort_column_comp = 8
data_sorted_comp = data_comp[data_comp[:,sort_column_comp].argsort()]
uni_comp, c_comp = np.unique(data_comp[:,sort_column_comp], return_counts=True)
binning_z = uni_comp.astype(np.float32)
...
print 'sep_arrays_comp', len(sep_arrays_comp)
for x in sep_arrays_comp:
    print '\tin', x.T.shape
    break

comp_array = []

for arr in sep_arrays_comp:
    arr = arr.T
    sort_column_comp = 7
    data_sorted_comp = arr[arr[:,sort_column_comp].argsort()]
    uni_comp, c_comp = np.unique(arr[:,sort_column_comp], return_counts=True)
    binning_y = uni_comp.astype(np.float32)
    ...
    sep_arrays_comp_2 = []
    for x in xrange(n_comp):
        sep_arrays_comp_2.append(col_array_comp[:,x])

    sub_array = []
    for arr_2 in sep_arrays_comp_2:
        arr_2 = arr_2.T
        sort_column_comp = 6
        data_sorted_comp = arr_2[arr_2[:,sort_column_comp].argsort()]
        uni_comp, c_comp = np.unique(arr_2[:,sort_column_comp], return_counts=True)
        binning_x = uni_comp.astype(np.float32)
        ...
        sep_arrays_comp_3 = []
        for x in xrange(n_comp):
            sep_arrays_comp_3.append(col_array_comp[:,x])
        sep_arrays_comp_3 = np.array(map(np.hstack, sep_arrays_comp_3))

        sub_array.append(sep_arrays_comp_3)
    comp_array.append(sub_array)

comp_array = np.array(comp_array)
print 'comp_array', comp_array.shape
...
for idx, array in enumerate(data_array):
    ...
    reduced_llh = array.T[9].astype(np.float32) - min_llh
    reduced_llh_comp = comp_array[idx].T[9].astype(np.float32) - min_llh_comp

    diff_llh = reduced_llh - reduced_llh_comp
    # diff_llh = (array.T[9].astype(np.float32) - comp_array[idx].T[9].astype(np.float32)) / array.T[9].astype(np.float32)
    print 'diff_llh', diff_llh.shape
    print 'diff_llh', diff_llh
    ...
    for xmaj in binning_x:
        ax.axvline(x=xmaj, ls=':', color='gray', alpha=0.7, linewidth=0.2)
    for ymaj in binning_y:
        ax.axhline(y=ymaj, ls=':', color='gray', alpha=0.7, linewidth=0.2)

    vlimits = (np.mean(diff_llh) - 1 * np.std(diff_llh), np.mean(diff_llh) + 1 * np.std(diff_llh))
    print 'vlimits', vlimits
    # arr = np.vstack([x, y, diff_llh]).astype(np.float32)
    # print arr.shape
    # im = ax.imshow(diff_llh, cmap='hot', interpolation='none')
    # im = ax.pcolor(x, y, diff_llh)
    cax = ax.pcolormesh(binning_x, binning_y, diff_llh, vmin=vlimits[0], vmax=vlimits[1], cmap=plt.get_cmap('RdBu'))
    cb = fig.colorbar(cax)
    cb.ax.tick_params(labelsize=11)
    cb.set_label(r'deviated_dLLH - nominal_dLLH', fontsize=11)
